...
@@ -160,40 +160,43 @@ public void paintComponent(Graphics g) {
...
//			xy.fill = "x="+x+", y="+y;
	...
	private static void quick1(double[] rate, double[] value, double[] weight, int beg, int end){
		if(beg<end){
	...
	//3.快速排序具体的过程
		public static void quick(double rate[], double value[], double weight[], int sta, int end){
			if(sta<end){
				int cent = partition1(rate, value, weight, sta, end);//在数组中的中心位置的点：cent
				quick1(rate, value, weight, sta, cent-1);//对数组中的前半部分进行排序
				quick1(rate, value, weight, cent+1, end);//对数组中的后半部分进行排序
			}
		}
	}
	private static int partition1(double[] rate, double[] value, double[] weight, int beg, int end){
		int pivot = end;
		int index = beg;
